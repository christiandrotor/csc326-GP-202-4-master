...
public class UserController {
     ...
     * @return response
     */
    @GetMapping ( value = "personnel/viewLogs" )
    @PreAuthorize ( "hasRole('ROLE_HCP') or hasRole('ROLE_PATIENT') or hasRole('ROLE_ADMIN')" )
    public String viewLogs ( final Model model ) {
        final User self = User.getByName( SecurityContextHolder.getContext().getAuthentication().getName() );
        final PersonnelForm form = new PersonnelForm( Personnel.getByName( self.getUsername() ) );
        model.addAttribute( "PersonnelForm", form );
        LoggerUtil.log( TransactionType.VIEW_LOGS, self );
        return "/personnel/viewLogs";
    }
    // Newly added
