    ...
    public ResponseEntity changePassword ( @RequestBody final PasswordChangeForm form ) {
        ...
        try {
            if ( form.validateChange( user ) ) {
                ...
                String body = "Hello " + firstName + ", \n\nWe receieved a request to change your password.\n";
                body += "MID: " + user.getId();
                EmailUtil.sendEmail( addr, "iTrust2 Password Change", body );

                try {
                    LoggerUtil.log( TransactionType.PASSWORD_CHANGE_SUCCESS, user.getUsername() );
                }
                catch ( final Exception e ) {
                    LoggerUtil.log( TransactionType.EMAIL_FAILED, user.getUsername() );
                }
