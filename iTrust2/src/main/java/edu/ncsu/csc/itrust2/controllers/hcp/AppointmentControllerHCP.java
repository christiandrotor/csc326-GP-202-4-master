...
public class AppointmentControllerHCP {
    ...
    public String appointmentActionSubmit ( @ModelAttribute final AppointmentForm form ) {
        ...
        if ( status == Status.PENDING ) {
            ...
            body = "Hello " + firstName + ", \n\nYour appointment status is rejected.\n";
        }

        try {
            EmailUtil.sendEmail( addr, "iTrust2 Appointment Status", body );
            LoggerUtil.log( TransactionType.APPOINTMENT_REQUEST_CHANGED, u.getUsername() );
        }
        catch ( final MessagingException e ) {
            LoggerUtil.log( TransactionType.EMAIL_FAILED, u.getUsername() );
        }
