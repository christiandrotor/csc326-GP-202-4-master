...
public class PasswordChangeStepDefs {
    ...
    @When ( "I receive an email with a confirmation of changed password" )
    public void getEmailPasswordChange () throws InterruptedException {
        // wait for the email to be sent
        try {
            new WebDriverWait( driver, 50 ).until( ExpectedConditions
                    .textToBePresentInElementLocated( By.name( "message" ), "Password changed successfully" ) );
        }
    ...
    private PasswordResetToken getTokenFromEmail () {
        ...
        try {
            ...
            for ( final Message message : messages ) {
                ...
                else if ( message.getSubject() != null && message.getSubject().contains( "iTrust2 Password Change" ) ) {
                    token = new PasswordResetToken();
                    break;
                }
            }
