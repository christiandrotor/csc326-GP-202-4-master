...
public class APIOfficeVisitTest {
    ...
    public void testPreScheduledOfficeVisit () throws Exception {
        ...
        // (student) test for patient
        final Long id = OfficeVisit.getForPatient( patient.getUsername() ).get( 0 ).getId();

        visit.setId( id.toString() );

        mvc.perform( post( "/api/v1/officevisits/patient/view/" + visit.getId() )
                .contentType( MediaType.APPLICATION_JSON ).content( TestUtils.asJsonString( visit ) ) )
                .andExpect( status().isOk() );
        mvc.perform( post( "/api/v1/officevisits/patient/view/-1" ).contentType( MediaType.APPLICATION_JSON )
    ...
    public void testOfficeVisitAPI () throws Exception {
        ...
        // (student) hcp
        mvc.perform( post( "/api/v1/officevisits/hcp/view/" + visit.getId() ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( visit ) ) ).andExpect( status().isOk() );
        mvc.perform( post( "/api/v1/officevisits/hcp/view/-1" ).contentType( MediaType.APPLICATION_JSON )
                .content( TestUtils.asJsonString( visit ) ) ).andExpect( status().isNotFound() );
    ...
    /**
     * Tests to retrieve the office visits from the database
     *
     * @throws Exception
     */
    // @WithMockUser ( username = "hcp", roles = { "HCP" } )
    // @Test
    // public void testOfficeVisitsHCP () throws Exception {
    // assertNotNull( mvc.perform( get( "/api/v1//officevisits/hcp/view/" + ) )
    // );
    // }
    ...
    @Test
    public void testOfficeVisits () throws Exception {
        assertNotNull( mvc.perform( get( "/api/v1/officevisits/myofficevisits" ) ) );
    }
